...
% the fit is limited to discrete discrete times
 ...
 case 'Single' % single exponential decay
  ...
  case 'DoubleFreeBKG'											% double exponential decay
  ...
  case 'DoubleBKG'										% double exponential decay (with background)
   ...
   startpoint = [ 0.9  Start_Gamma1 0.1  Start_Gamma2 0     ]; %        changed 1    from 0.9 to 1
  ...
  case 'Cumulants'
   ...
   case 'Cumulants2BKG' % cumulants (Pursey, Frisken) with nonlinear fit methodwhich is also stable for higher taus
	fit_function = 'b + A * exp( - 2 * Gammac * t) .*( 1 + mu2 / 2 * t .^2 ).^2';
   coeffnames = {'b'   'A' 'Gammac'     'mu2'};
   lowerbond  = [-1e-3 0.8 Min_Gamma2   -1e3];
   upperbond  = [1e3   1.2 Max_Gamma1   1e3];
   startpoint = [0     1   Start_Gamma1 0.1];
   ...
   lowerbond  = [0.01 Min_Gamma1   0    Min_Gamma2   -1e3  -1e-3 ] ;
